...
const phasesPrompt = (cw) => `use a workflow. In ${REPO}, run the saved workflow: Workflow({ name: '${WORKFLOW}', args: ${JSON.stringify(cw)} }). The Workflow tool returns IMMEDIATELY with a task id and runs in the background; if you end your turn the workflow is killed. So after calling it, block on it: call TaskOutput({ task_id, block: true }) (load it with ToolSearch if needed) and keep calling it until it reports the workflow has completed — shifts run for hours, so keep waiting through every timeout, never end your turn early, never say you will report later. Only when the workflow has completed, from its return value, build the state block: continueWith = its continueWith (null if nothing remains), done = false, resetAt = its resetAt if it returned paused. Summarise merged phases and flags in two lines above the block.${POLICY}`
